...
This catalog groups existing and planned test cases by validation layer. Test code stays in module-owned `tests/` folders; this document is the cross-cutting index under `openspec/docs/validation/`.
...
| Grafana artifact validation | `scripts/validate_grafana_artifacts.py` | Validates committed JSON artifacts against `openspec/docs/current-baseline/grafana-approved-data-surfaces.json`. |
...
| Approved data surface | `openspec/docs/current-baseline/grafana-approved-data-surfaces.json` | Declares allowed Metrics API paths and optional `chart_id` for chart-data. |
...
| C0 runtime closure | `openspec/docs/validation/c0-validation-closure-evidence.md`, `scripts/check_c0_validation_evidence.py` | Reference UI click/clear, live API checks, local Grafana render, checker-enforced runtime closure. |
| C1 evidence link | `openspec/docs/validation/c1-evidence-link-validation-evidence.md`, `scripts/check_c1_evidence_link_evidence.py` | Grafana link target resolution, row count/title match against reference Metrics evidence, C-stock link-out decision. |
...
| C0/C1 evidence is checked but evidence generation is partly manual/local | Add script to regenerate C0/C1 evidence from live local services. | `scripts/` plus docs under `openspec/docs/validation/`. |
| Cross-contract coverage matrix is currently documented, not machine-checked | Add a lightweight checker that ensures each authority field in `openspec/docs/validation/test-strategy.md` has named test files. | `scripts/check_validation_matrix.py` if this becomes a recurring release gate. |
