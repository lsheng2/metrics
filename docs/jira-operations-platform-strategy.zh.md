...
也就是说，我们要做的不是 Python 版 `jira-cli`，也不是 `JiraOperationsPlatform`、`GitHubOperationsPlatform`、`AzureOperationsPlatform` 三套平行系统。正确方向是 provider-neutral core 加 provider-specific adapters：共享 identity、query/filter、metadata、fact projection、AI action governance、audit 和 dashboard consumption 模型；Jira、GitHub、Azure DevOps 等 provider 只实现自己具备的 capability。
...
目标：AI 可以提出 work item 修改建议，但用户确认前不能写。Jira adapter 支持 issue update/transition/comment；GitHub adapter 可支持 issue comment、labels、milestone、assignee、close/reopen 等 capability。
