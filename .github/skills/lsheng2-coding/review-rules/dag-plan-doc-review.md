...
- Treat `docs/implementation-start.md`, `docs/architecture-manual.md`, and `docs/bug-trend-architecture-spec.md` as the current Intel Jira bug trend MVP authority set when a plan touches `jira_sync/`, `jira_history/`, `bug_metrics/`, `ui_web/`, `ops/`, or Grafana artifacts.
...
- Use repo-relative owner paths from the project profile. Valid families include module roots, module-local tests, `docs/`, `ops/`, `scripts/`, `.github/`, `ui_web/templates/`, and `ui_web/static/` only when the node explicitly owns them.
...
- Every plan that changes bug trend evidence, export, chart catalog, Grafana JSON, parity scripts, audit behavior, or AI chart governance must name both producer and consumer surfaces, including relevant `ops/grafana/`, `scripts/`, `docs/`, and UI/API paths.
...
- Require findings for code-doc truth drift across `CLAUDE.md`, `.github/copilot-instructions.md`, `.github/ai-governance/`, `README.md`, `docs/`, project agents, and project skill profile files when the plan changes stable behavior.
